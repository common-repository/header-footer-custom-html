 ...
 */
if ( ! class_exists( 'Enwb_Hfch_Boo_Settings_Helper' ) ):

	class Enwb_Hfch_Boo_Settings_Helper {
...
//			$this->set_default_db_options();
			...
			$this->script_general();
			do_action('after_boo_admin_settings');
			?>
            </div>
			<?php			
		}
...
//								'type'              => $field['type'],
		...
		// RF htmleditor code
		function sanitize_htmleditor( $value ) {
...
//		    TODO: if the option to store file as file url
		...
		/**
		 * Displays a htmleditor for a settings field
		 *
		 * @param array $args settings field args
		 */
		...
		function callback_htmleditor( $args ) {
			/*rf function*/
			$html = sprintf(
				'<textarea 
                        rows="5" 
