...
if ( ! class_exists( 'Enwb_Hfch_Notices' ) ) :
	...
	class Enwb_Hfch_Notices {
		 ...
		 * @since 1.0.0
		 */
		private $plugin_name_dashed = HEADER_FOOTER_CUSTOM_HTML_PLUGIN_NAME;
		 ...
		 * @since 1.0.0
		 */
		private static $version = HEADER_FOOTER_CUSTOM_HTML_PLUGIN_VERSION;
		...
		public function __construct() {
			add_action( 'admin_notices', array( $this, 'show_notices' ), 100 );
			add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
			add_action( 'wp_ajax_enwb-notice-dismiss', array( $this, 'dismiss_notice' ) );
			add_filter( 'wp_kses_allowed_html', array( $this, 'add_data_attributes' ), 10, 2 );
			//add_action('admin_init', array( $this, 'remove_notice_transient') );
		}
		...
		public function dismiss_notice() {
			...
			if ( ! apply_filters( 'enwb_notices_user_cap_check', current_user_can( $capability ) ) ) {
				return;
			}

			if ( false === wp_verify_nonce( $nonce, 'enwb-notices' ) ) {
				wp_send_json_error( esc_html_e( 'WordPress Nonce not validated.', $plugin_name_dashed ) );
			}
		...
		public function enqueue_scripts() {
			wp_enqueue_style( $this->plugin_name_dashed.'-notices', self::get_url() . 'notices.css', array(), self::$version );
			wp_enqueue_script( $this->plugin_name_dashed.'-notices', self::get_url() . 'notices.js', array( 'jquery' ), self::$version, true );
			wp_localize_script(
				$this->plugin_name_dashed.'-notices',
				'enwbNotices',
				array(
					'_notice_nonce' => wp_create_nonce( 'enwb-notices' ),
				)
			);
		}
		...
		public function show_notices() {
			$defaults = array(
				'id'                         => '',      // Optional, Notice ID. If empty it set `enwb-notices-id-<$array-index>`.
			...
			foreach ( $notices as $key => $notice ) {
				...
				if ( isset( $notice['show_if'] ) && true === $notice['show_if'] ) {
					...
					//if ( false !== self::is_notice_valid( $notice ) ) {
		...
		public static function markup( $notice = array() ) {
			wp_enqueue_script( 'enwb-notices' );
			wp_enqueue_style( 'enwb-notices' );

			do_action( 'enwb_notice_before_markup' );

			do_action( "enwb_notice_before_markup_{$notice['id']}" );

			?>
			<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="<?php echo 'enwb-notice-wrapper ' . esc_attr( $notice['classes'] ); ?>" data-repeat-notice-after="<?php echo esc_attr( $notice['repeat-notice-after'] ); ?>">
				<div class="enwb-notice-container">
					<?php do_action( "enwb_notice_inside_markup_{$notice['id']}" ); ?>
					<?php echo wp_kses_post( $notice['message'] ); ?>
				</div>
			</div>
			<?php

			do_action( "enwb_notice_after_markup_{$notice['id']}" );

			do_action( 'enwb_notice_after_markup' );
		}
		...
		private static function get_wrap_classes( $notice ) {
			$classes = array( 'enwb-notice', 'notice' );
		...
		/*function remove_notice_transient() {
		...
		/**
		 * Get base URL for the enwb-notices.
		 *
		 * @return mixed URL.
		 */
	 ...
	 */
	Enwb_Hfch_Notices::get_instance();
